...
async function loadData(){
  try{
    const [q,p,ph,st] = await Promise.all([
      fetch('questions_ipp_v3.json').then(r=>{ if(!r.ok) throw new Error('questions_ipp_v3.json'); return r.json(); }),
      fetch('profiles_36_ipp_v3.json').then(r=>{ if(!r.ok) throw new Error('profiles_36_ipp_v3.json'); return r.json(); }),
      fetch('phase_items_v3.json').then(r=>{ if(!r.ok) throw new Error('phase_items_v3.json'); return r.json(); }),
      fetch('stress_items_v3.json').then(r=>{ if(!r.ok) throw new Error('stress_items_v3.json'); return r.json(); })
    ]);
    questions=q; profiles=p; phaseItems=ph; stressItems=st;
  } catch(err){
    const app=document.getElementById('app');
    app.innerHTML = `<section class="hero"><div><span class="pill">IPP A4P</span><h1>Impossible de charger le questionnaire</h1><p>Certains fichiers du dossier ne sont pas accessibles. Vérifie que tous les fichiers du pack sont bien déposés à la racine du dépôt GitHub Pages.</p><p class="muted">Fichier concerné : ${err.message || 'chargement des données'}</p></div></section>`;
    throw err;
  }
}
...
function renderReport(){
  ...
  html += page("Page 15 — Stress actuel", `
    ...
    <p>Le module stress met en évidence les zones où ton fonctionnement peut se crisper quand la pression augmente. Il aide à prioriser les leviers de régulation les plus utiles pour toi.</p>
  `);
